...
function showSnacks() {
  ...
  let creamyDrinks = drinkArray.filter(b =>{ 
    if (b.fields){
      return true; 
    } else {
      return false;
  ...
  let filter1 = creamyDrinks.filter(b =>{   
  if (b.fields.type.includes('creamy') &&(b.fields.color.includes('yellow')||b.fields.color.includes('green'))) {
    return true;
  } else {
    return false
  }
  ...
  if (a.fields.type.includes('savory') &&(a.fields.color.includes('yellow')||a.fields.color.includes('green'))) {
    return true;
  } else {
    return false
  }
